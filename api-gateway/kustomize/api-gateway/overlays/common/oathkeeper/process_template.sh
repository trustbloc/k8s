...
echo
config_map=$(kubectl get cm  -l component=gateway  -o jsonpath='{range .items[*]}{.metadata.name}{"\n"}{end}' | grep oathkeeper-rules)
...
echo "labeling"
kubectl label cm ${config_map} component=gateway group=services project=trustbloc instance=||DEPLOYMENT_ENV||
